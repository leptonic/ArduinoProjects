...
void setup() {
  ...
  // lazer part
  Serial.begin(9600);
  pinMode(PIN_LAZER, OUTPUT);  //   定义12脚为数字输出接口
...
void wifi_set_deafult()
{
  ...
  delay(2000);
  Serial.print("AT+CWSAP=\"DBv1\",\"Avadd833629\",2,2\r\n");
  wdt_reset();
  delay(5000);
  wdt_reset();
  Serial.print("AT+RST\r\n");
...
void beep()
{
 digitalWrite(PIN_BEEP, HIGH); // close relay
 delay(100);
 wdt_reset();
  digitalWrite(PIN_BEEP, LOW); // close relay
  delay(200);
  wdt_reset();
   digitalWrite(PIN_BEEP, HIGH); // close relay
 delay(300);
